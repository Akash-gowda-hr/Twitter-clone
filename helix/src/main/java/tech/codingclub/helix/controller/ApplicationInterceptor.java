...
public class ApplicationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object handler) throws Exception {
        ...
        if(httpServletRequest.getRequestURI().equals("/welcome"))
        {
            ...
            if(isUserLogined != null && isUserLogined)//basically it is true
            {
                return true;
            }
            else
            {
                //this is case when login was failed
                httpServletResponse.sendRedirect("/login/welcome?failed=1x");
                return false;//this cancels the previous request which was made and it redirest to same page
            }

        }

        //below code makes all code within user controller is guarded
